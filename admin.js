...
document.querySelector('#login-form').onsubmit=async e=>{e.preventDefault();const st=document.querySelector('#login-status');st.className='form-status show ok';st.textContent='Enviando acceso…';const{error}=await sb.auth.signInWithOtp({email:EMAIL,options:{shouldCreateUser:false,emailRedirectTo:'https://srvegaa.github.io/de-otra-epoca/admin.html'}});st.className='form-status show '+(error?'error':'ok');st.textContent=error?'No se pudo enviar el acceso. Esperá un minuto antes de intentar otra vez.':'Revisá tu correo y abrí el enlace seguro.'};
